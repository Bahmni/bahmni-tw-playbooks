...
cd bahmni-playbooks && ansible-playbook --syntax-check all.yml concat-config.yml incr-mysqldbbackup.yml incr-mysqldbrestore.yml incr-postgresdbbackup.yml incr-postgresdbrestore.yml backup-artifacts.yml restore-artifacts.yml mysql-replication.yml postgres-replication.yml -i local && cd ..
...
sh ./bahmni-package/bahmni-installer/scripts/generate-bahmni-version.sh
cp rpm_versions.yml bahmni-package/bahmni-installer/resources
#Working directory: bahmni-package
cd bahmni-package && ./gradlew -PbahmniRelease=${rpm_version} :bahmni-installer:clean :bahmni-installer:buildRpm